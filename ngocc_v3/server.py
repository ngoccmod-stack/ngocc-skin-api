...
def ensure_local_resources_from_cloud(version: str | None = None) -> str:
    """Restore the requested/latest persisted Resources ZIP from Cloudinary into local ephemeral disk."""
    versions = find_resource_versions(RESOURCES)
    if version:
        local = RESOURCES / version
        if (local / "Databin/Client/Actor/heroSkin.bytes").is_file():
            return version
    else:
        if versions:
    ...
    target_version = version or manifest.get("active") or ""
    if not target_version:
        raise FileNotFoundError("Chưa có Resources; Admin hãy cập nhật Resources trước.")
    info = manifest.get("versions", {}).get(target_version) or {}
    url = info.get("secureUrl") or resource_secure_url(target_version)
    if not url:
        raise FileNotFoundError(f"Không tìm thấy bản Resources {target_version} đã lưu.")

    local_zip = UPLOADS / f"restore_{target_version}.zip"
    ...
    with local_zip.open("wb") as f:
        for chunk in r.iter_content(1024 * 1024):
            if chunk:
                f.write(chunk)
    cmd = [sys.executable, str(AUTOMOD / "resource_manager.py"), "--resources", str(RESOURCES), "install", str(local_zip)]
    cp = subprocess.run(cmd, cwd=str(ROOT), capture_output=True, text=True, timeout=900)
    try:
        if cp.returncode != 0:
            raise RuntimeError(cp.stderr or cp.stdout or "Không thể khôi phục Resources")
        installed = re.search(r"Installed Resources version:\s*(.+)", cp.stdout)
        installed_version = installed.group(1).strip() if installed else target_version
...
async def upload_resources(file: UploadFile = File(...)):
    if not file.filename or not file.filename.lower().endswith(".zip"):
        raise HTTPException(400, "Chỉ nhận file Resources .zip")
    if not cloudinary_ready():
        raise HTTPException(503, "Chưa cấu hình CLOUDINARY_URL trên Render để lưu Resources lâu dài.")

    target = UPLOADS / f"incoming_{Path(file.filename).name}"
    try:
        with target.open("wb") as f:
            while chunk := await file.read(1024 * 1024):
                f.write(chunk)

        # Detect version from ZIP before uploading.
        cp = subprocess.run(
            [sys.executable, str(AUTOMOD / "resource_manager.py"), "--resources", str(RESOURCES), "install", str(target)],
            cwd=str(ROOT), capture_output=True, text=True, timeout=900
        )
        if cp.returncode != 0:
            raise HTTPException(400, cp.stderr or cp.stdout or "Không thể cài Resources")
        m = re.search(r"Installed Resources version:\s*(.+)", cp.stdout)
        version = m.group(1).strip() if m else ""
        if not version:
            raise HTTPException(400, "Không xác định được version Resources")

        # Upload original ZIP to Cloudinary as a raw asset. upload_large is required for >100MB files.
        upload_result = cloudinary.uploader.upload_large(
            str(target),
            resource_type="raw",
            public_id=resource_public_id(version),
            overwrite=True,
            chunk_size=20 * 1024 * 1024,
        )
        secure_url = upload_result.get("secure_url") or resource_secure_url(version)
        manifest = load_resource_manifest()
        manifest.setdefault("versions", {})[version] = {
            "version": version,
            "secureUrl": secure_url,
            "publicId": resource_public_id(version),
            "uploadedAt": __import__("datetime").datetime.now(__import__("datetime").timezone.utc).isoformat(),
            "size": target.stat().st_size,
        }
        manifest["active"] = version
        save_resource_manifest(manifest)
        save_json(ACTIVE, {"version": version, "installedAt": __import__("datetime").datetime.now(__import__("datetime").timezone.utc).isoformat()})
        return {"ok": True, "version": version, "stored": True, "url": secure_url}
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(500, f"Cập nhật Resources thất bại: {e}")
    finally:
