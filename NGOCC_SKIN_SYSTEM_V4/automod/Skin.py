...
# Web-build mode: run a single skin build non-interactively.
...
if WEB_BUILD_MODE:
    ...
    def _ngocc_web_input(prompt=""):
        pl = prompt.lower()
        if "other function" in pl:
            return "n"
        if "cách thức nhập id" in pl:
            return "1"
        if "id skin" in pl:
            return WEB_BUILD_ID
        if "enter skin pack name" in pl:
            return WEB_BUILD_NAME.replace(" [DiaoChan]", "")
        if "mod component" in pl:
            return "3"
        if "special: 54402" in pl:
            return "n"
        if "thông báo hạ nakroth" in pl and "use skin producer" in pl:
            return "2"
        if "thông báo hạ:" in pl:
            return "1"
        return "n"
    _builtins.input = _ngocc_web_input
