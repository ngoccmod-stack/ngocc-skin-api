...
class BundleFile(File.File):
    ...
    def read_fs(self, reader: EndianBinaryReader):
        ...
        # Keep the original archive flags so an edited bundle can be written
        # back with the same layout/encryption flags.
        self._data_flags = int(self.dataflags)
    ...
    def save_fs(self, writer: EndianBinaryWriter, data_flag: int, block_info_flag: int):
        ...
        # AOV encrypts both the compressed block-info and the compressed asset
        # data with SM4 after compression. Encryption does not change their size.
        if self._is_encrypted(data_flag):
            block_data = self._encrypt_block(block_data)
            file_data = self._encrypt_block(file_data)
        ...
        # Rewrite the 16-byte UnityFS size/compression header. AOV protects this
        # header with the same AES-CBC transform used by decryptHeader().
        if self._is_encrypted(data_flag):
            header = b""
    ...
    def _is_encrypted(self, data_flag: int) -> bool:
        """Return whether the bundle uses the AOV/Unity asset-bundle encryption bit."""
        encryption_flag = int(self.dataflags.UsesAssetBundleEncryption)
        return bool(data_flag & encryption_flag)

    def _encrypt_block(self, data: bytes) -> bytes:
        """Encrypt the 16-byte-aligned portion of an AOV data block with SM4.

        AOV leaves the final partial block (when the length is not a multiple of
        16) untouched, matching the existing decryptor implementation.
        """
        if not data:
            return data
        if self.decryptor is None:
            self.decryptor = ArchiveStorageManager.ArchiveStorageDecryptor()
        blocksize = (len(data) // 16) * 16
        if blocksize == 0:
            return data
        key0 = SM4Key(self.decryptor.BlockSM4Key)
        encrypted = key0.encrypt(
            data[0:blocksize], initial=bytearray(self.decryptor.BlockSM4IV)
        )
        return encrypted + (data[blocksize:] if blocksize < len(data) else b"")
    ...
    def decompress_data(
        ...
        if comp_flag == CompressionFlags.LZMA:  # LZMA
            ...
            # For newer Unity bundles the encryption bit is 0x400; for older
            # bundles it is 0x200.  Use the enum selected while reading the
            # bundle instead of a hard-coded bit.
            encryption_flag = int(self.dataflags.UsesAssetBundleEncryption)
            if (int(self.dataflags) & encryption_flag) and self.decryptor:
