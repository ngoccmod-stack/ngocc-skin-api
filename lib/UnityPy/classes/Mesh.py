...
class Mesh(NamedObject):
    ...
    def __init__(self, reader):
        ...
        if version < (3, 5):  
            ...
            self.m_Vertices = reader.read_float_array(self.m_VertexCount * 3)  

            self.m_SkinSize = reader.read_int()
            self.m_Skin = [BoneWeights4(reader) for _ in range(self.m_SkinSize)]

            self.m_BindPose = reader.read_matrix_array()
            self.m_UV0 = reader.read_float_array(reader.read_int() * 2)  
            self.m_UV1 = reader.read_float_array(reader.read_int() * 2)  
        ...
        if version[:2] <= (3, 4):  
            ...
            reader.Position += m_CollisionTriangles_size * 4  
...
class MeshHelper:
    ...
    def ToVertexFormat(format: int, version: List[int]) -> "VertexFormat":
        if version[0] < 2017:
            ...
            elif format == VertexChannelFormat.kChannelFormatColor:  
            ...
            elif format == VertexChannelFormat.kChannelFormatUInt32:  
